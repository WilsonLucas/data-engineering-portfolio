...
Na Dataside, desde 2023, a atuação em modelo de consultoria itinerante é o que melhor representa onde estou hoje: cada projeto começa do zero em termos de contexto de negócio, mas o método é consistente — diagnóstico rigoroso do ambiente, arquitetura baseada em padrões comprovados, implementação auditada, transferência de conhecimento.

A frente mais longa dessa fase trouxe um desafio diferente: sustentação de missão crítica em um ambiente Oracle corporativo de uma empresa de energia, consolidado ao longo de anos, ao mesmo tempo em que se conduz a modernização da camada semântica para SAP Datasphere. É o tipo de trabalho onde a habilidade de não quebrar o que funciona é tão valiosa quanto a habilidade de construir o que é novo.

O ano de 2026 ampliou esse método para fora do Azure. Uma migração de lakehouse inteiro do Azure Synapse para Google Cloud (Apache Iceberg, BigQuery, Cloud Composer, Dataproc Serverless) exigiu um contrato de camadas novo e uma ferramenta própria de monitoria que mede o ambiente antigo ao vivo. Uma virtualização de SAP Datasphere em Databricks via Lakehouse Federation, promovida por esteira de CI/CD em três ambientes, foi para produção em setembro. E um programa de FinOps em Azure mostrou que a mesma disciplina de evidência antes de recomendação vale para custo tanto quanto para dado.
...
**Governança e Unity Catalog.** A catalogação de 433+ tabelas com controle de acesso por ambiente (dev vs. prod) em Unity Catalog mudou a maneira como penso sobre discovery de dados e lineage. Não é só uma feature do Databricks — é a diferença entre um data lake que vira data swamp e uma plataforma onde as pessoas confiam no que encontram. A linhagem de runtime do Unity Catalog também virou instrumento de engenharia: foi ela que confirmou as 76 arestas do grafo de dependências descoberto do código de uma seguradora, com zero falsos positivos.

**Migração entre nuvens e formatos abertos.** Migrar 12,4 TB de Delta Lake no Azure Synapse para Apache Iceberg e BigQuery no Google Cloud ensinou que a parte difícil não é o formato — é decidir o que a plataforma antiga fazia de errado e não deve ser reproduzido. Um predicado herdado que era tautologia na origem e zerava uma dimensão no destino, 126 mil unidades fantasma projetadas de uma tabela associativa, 100 mil linhas cartesianas: cada divergência precisa ser classificada como defeito do alvo ou defeito da migração antes de ser "corrigida".

**FinOps como engenharia.** Reduzir custo de nuvem com a mesma disciplina de homologação: 90 dias de métricas em vez de 30 para descartar sazonalidade, mapeamento de dependências antes de qualquer exclusão, ARM export para reversibilidade e a regra de que zero em uma consulta sob permissão restrita não é ausência, é cegueira.
...
**Virtualização vs. materialização:** Uma view federada sobre SAP Datasphere entrega o dado sem cópia, sem job de carga e sem janela de inconsistência, ao custo de latência de consulta e dependência da disponibilidade da origem. Materializar em Delta com MERGE agendado inverte o trade-off. A escolha certa depende da pergunta que deveria vir primeiro: a demanda pede cópia ou pede acesso? Em 2026 matei um workstream inteiro de materialização, já justificado por escrito, porque a resposta era acesso.
...
*Para detalhes de contato e links profissionais, ver a [seção de contato do portfolio](./index.html#contato).*
