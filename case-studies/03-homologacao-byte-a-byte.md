...
**Período:** Março a junho de 2026. Continuação no [Case 06](06-migracao-synapse-gcp-lakehouse.md) (migração para Google Cloud)
...
## Refinamento Forense da Dimensão de Atendimento (abril a junho de 2026)

Depois da homologação inicial, a dimensão mais crítica do ambiente passou por investigação forense coluna a coluna contra a base on-premise: 80 colunas no schema, 75 comparáveis, 53 sem divergência, 4 divergências reais, 11 indeterminadas e 7 não classificadas. Os ofensores reais eram flags de aceite (34,2%) e de pagamento (17,9%) e CEP (4,4%).

Dez correções foram aplicadas em produção. Melhorias medidas em três campos descritivos: divergência de tipo de plano de 68% para 0,19%, de canal de 62% para 5,98% e de tipo de 59% para 1,36%.

Desse ciclo nasceu uma regra inviolável antes de editar qualquer notebook: teste de parse e teste semântico via OPENROWSET com amostra de pelo menos mil ids. A regra foi motivada por duas falhas históricas: um commit que quebrou o ambiente de desenvolvimento e um `MAX` em tabela de ministrantes que inflou uma categoria em 2.751%.

Uma segunda análise forense cruzou a tabela nacional de atendimentos com a dimensão local: de 381.475 chaves do estado, apenas 17,71% eram cobertas por qualquer dos 4 ids disponíveis. O gap de 82,29% foi a evidência que sustentou a redefinição do MATCH de identidade na camada Ouro da nova plataforma, documentada no Case 06.
