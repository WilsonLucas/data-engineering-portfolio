...
**Período:** Março a julho de 2026 (MVP entregue em abril, encerramento operacional em julho)
...
## Encerramento e Governança (abril a julho de 2026)

O MVP de 7 blocos foi entregue em 23 de abril, em três ondas, com todas as 8 tabelas fato homologadas pelo cliente e 50 dos 50 indicadores da especificação cobertos. Após a entrega, o trabalho passou para governança do lakehouse e encerramento operacional.

**Contagens homologadas pelo cliente:** benefícios 23.821.666 linhas, dependentes 368.780, pagamentos 10.849.242, encargos 3.089.011, jornada 1.321.273, rescisões 348.155, horas extras 357.172, banco de horas 44.419, endividamento 15.183. Total de cerca de 40,2 milhões de linhas.

**Política de VACUUM por camada.** 439 de 449 tabelas protegidas via propriedades de tabela (Raw 180 dias, Trusted 30 dias, Refined 14 dias) com marcador explícito de exclusão. 10 tabelas zumbi removidas e 20 paths órfãos de storage apagados, após classificação read-only do metastore.

**Quality gates mensais.** Quatro asserções gravadas em tabela de auditoria (uma crítica de aderência estrita à especificação, três de aviso para verbas não classificadas e lacunas), mais um guard de origem em 10 notebooks Refined e 2 Trusted que bloqueia primeira carga vazia e sai com aviso em incremental vazio.

**Bloqueio 17 e a lição do decimal brasileiro.** Uma tabela Trusted estava com 94% de nulos porque `"1.234,56"` vira NULL em cast direto. A tabela foi reconstruída com 773.711 linhas e 0% de nulos, schema migrado para DECIMAL(14,3) e backup por DEEP CLONE. A mesma causa explicava o bloqueio 02, resolvido meses antes por sintoma.

**Bug de framework reportado à plataforma.** A função compartilhada de carga não propaga colunas novas no `UPDATE SET` do MERGE. Reportado ao time de plataforma da consultoria com workaround manual de ALTER e backfill documentado.

**Lição de governança registrada.** Ausência no controller não significa objeto sem uso: 4 views que pareciam órfãs pertenciam a outro projeto no mesmo storage. Nenhuma remoção sem confirmação cruzada com o arquiteto.
